...
provide('expandedCommentIds', expandedCommentIds)
...
const findRootCommentById = (commentList: Comment[], commentId: number): Comment | null => {
  for (const comment of commentList) {
    if (comment.id === commentId) {
      return comment
    }
...
const handleReply = async (data: { content: string; parentId: number; replyToUserId?: number }) => {
  try {
    ...
    if (comment.status === 'approved') {
      const rootComment = findRootCommentById(comments.value, data.parentId)
      if (rootComment) {
