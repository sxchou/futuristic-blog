...
const triggerUpload = async () => {
  if (!await requirePermission('settings.edit', '上传网站Logo')) return
...
const handleReset = async () => {
  if (!await requirePermission('settings.edit', '恢复默认Logo')) return
    ...
    <div class="flex items-start gap-6">
      <div class="flex-shrink-0">
        <div
          ...
          class="rounded-full overflow-hidden bg-gray-100 dark:bg-dark-200 border border-gray-200 dark:border-white/10 flex items-center justify-center"
          :class="sizeClasses"
        >
          <img
        ...
        <div
          v-else
          class="rounded-full bg-black flex items-center justify-center relative overflow-hidden shadow-lg"
          :class="sizeClasses"
        >
          <svg
            viewBox="0 0 100 100"
