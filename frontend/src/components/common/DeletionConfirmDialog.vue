    ...
    >
      ...
      >
        ...
        >
          <div
            v-if="visible"
            class="relative bg-gray-900 dark:bg-dark-50 border border-gray-700 dark:border-white/10 rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden"
          >
            <div class="p-6">
              <div class="flex items-start gap-4">
                <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center bg-red-500/10">
                  <svg
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-gray-300 text-sm">
                    您即将删除 <span class="text-white font-medium">{{ preview?.item_name }}</span>
                  </p>
                </div>
              </div>
              ...
              >
                <div class="flex items-start gap-2">
                  ...
                  <p class="text-red-300 text-sm">{{ preview.block_reason }}</p>
                </div>
              </div>
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto custom-scrollbar">
                  <template v-for="(items, action) in groupedItems" :key="action">
                    <div
                      ...
                      class="rounded-lg bg-gray-800/50 dark:bg-dark-200/50 overflow-hidden"
                    >
                      <div class="flex items-start gap-3 p-2.5">
                        <div
                          :class="[
                            ...
                            item.action === 'unlink' ? 'bg-amber-400' :
                            item.action === 'soft_delete' ? 'bg-orange-400' : 'bg-yellow-400'
                          ]"
                        />
                        <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                          <span
                            v-if="item.count > 1"
                            class="text-xs text-gray-500 bg-gray-700/50 px-1.5 py-0.5 rounded"
                          >
                            ×{{ item.count }}
                          </span>
                          <button
                            v-if="item.count > 0"
                            class="text-xs text-blue-400 hover:text-blue-300 transition-colors p-1 rounded hover:bg-blue-400/10"
                      ...
                      >
                        <div
                          ...
                          class="border-t border-gray-700/50 dark:border-white/5"
                        >
                          ...
                          <div v-else-if="getDetailState(item).error" class="py-2 px-3">
                            <span class="text-xs text-gray-500">加载失败，请重试</span>
                          </div>
                          <div v-else-if="getDetailState(item).items.length === 0" class="py-2 px-3">
                            <span class="text-xs text-gray-500">无明细</span>
                          </div>
                          <div v-else class="py-1.5 px-3">
                            <div
                              v-for="(detail, idx) in getDetailState(item).items"
                              :key="idx"
                              class="flex items-center gap-2 py-1.5 border-b border-gray-700/30 dark:border-white/5 last:border-0"
                            >
                              <div class="w-1 h-1 rounded-full bg-gray-500 flex-shrink-0" />
                              <span class="text-xs text-gray-300 truncate flex-1">{{ detail.name }}</span>
                            ...
                            >
                              ...
                              <div class="flex items-center gap-1">
                                <button
                                  ...
                                  class="text-xs text-blue-400 hover:text-blue-300 px-1.5 py-0.5 rounded hover:bg-blue-400/10 transition-colors"
                            ...
                            <div
                              ...
                              class="pt-1 pb-0.5"
                            >
                              <span class="text-xs text-gray-500">共 {{ getDetailState(item).total }} 项</span>
                            </div>
                          </div>
                        </div>
                      </Transition>
              ...
              <div
                ...
                class="mt-4 p-3 rounded-lg bg-gray-800/50 dark:bg-dark-200/50"
              >
                <p class="text-sm text-gray-400">此操作没有关联数据受影响，删除后无法恢复。</p>
              </div>
            </div>
            ...
            <div class="flex border-t border-gray-700 dark:border-white/10">
              ...
              <button
                ...
                type="button"
                class="flex-1 px-4 py-3 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-colors disabled:opacity-50"
                :disabled="loading"
                @click="handleConfirm"
              >
