...
const { checkPermission, confirmPermission } = usePermission()
...
const handleExportClick = () => {
  if (!checkPermission('dashboard.export')) {
    confirmPermission('dashboard.export', '导出图表')
  }
}

const exportChart = async (type: 'png' | 'svg') => {
  if (!await confirmPermission('dashboard.export', '导出图表')) return
...
const exportData = async (type: 'csv') => {
  if (!await confirmPermission('dashboard.export', '导出数据')) return
