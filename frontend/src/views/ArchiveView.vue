...
let archiveCache: { data: YearData[]; timestamp: number } | null = null
...
const fetchArchive = async (force = false) => {
  if (!force) {
    ...
    const now = Date.now()
    if (archiveCache && now - archiveCache.timestamp < CACHE_TTL) {
      archiveData.value = archiveCache.data
      loading.value = false
      return
    }
  }
  ...
  try {
    ...
    archiveCache = { data: response.data, timestamp: Date.now() }
...
<template>
  <div class="flex flex-col lg:flex-row gap-6">
    ...
    <div class="flex-1 min-w-0 lg:order-2">
      ...
      <form class="flex flex-wrap items-center gap-2 mb-6" @submit.prevent>
        <div class="relative">
          <input id="input-archive-search"
            v-model="searchQuery"
            type="text"
            name="archive-search"
            placeholder="搜索文章..."
