...
<template>
  <div class="flex flex-col lg:flex-row gap-8">
    <div class="flex-1 min-w-0">
      <div class="mb-8">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2">标签云</h1>
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          >
            <div class="text-2xl font-bold mb-1 transition-colors" :style="{ color: tag.color }">
              {{ tag.article_count }}
            </div>
            <div class="text-sm text-gray-500 dark:text-gray-400 group-hover:text-primary transition-colors">#{{ tag.name }}</div>
          </router-link>
        </div>
      </div>

      <div v-if="blogStore.tags.length === 0" class="text-center py-16">
