...
<template>
  <div class="flex flex-col lg:flex-row gap-8">
    <div class="flex-1 min-w-0">
      <div class="mb-8">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2">搜索文章</h1>
      ...
      <div v-else-if="route.query.q && blogStore.articles.length === 0" class="text-center py-16">
        <svg class="w-12 h-12 mx-auto mb-3 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-1">未找到相关结果</h2>
        <p class="text-gray-500 dark:text-gray-400 text-sm">尝试使用不同的关键词搜索</p>
      </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0 flex flex-col">
              ...
              <p class="article-card-excerpt">
                ...
                <template v-else>{{ article.summary }}</template>
              </p>
      ...
      <div v-else class="text-center py-16">
        ...
        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-1">开始搜索</h2>
        <p class="text-gray-500 dark:text-gray-400 text-sm">输入关键词搜索文章、标签、分类</p>
      </div>
    </div>
