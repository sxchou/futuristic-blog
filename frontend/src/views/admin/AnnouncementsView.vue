...
const fetchAnnouncements = async () => {
  ...
  try {
    ...
    showMessage(error.response?.data?.detail || '获取公告失败', 'error')
  } finally {
...
const handleSave = async () => {
  ...
  try {
    ...
    showMessage(error.response?.data?.detail || '操作失败', 'error')
  } finally {
...
const handleDelete = async (id: number) => {
  if (!await requireAdmin('删除公告')) return
...
const toggleActive = async (announcement: Announcement) => {
  if (!await requireAdmin('修改公告状态')) return
  
  try {
    await announcementApi.updateAnnouncement(announcement.id, {
      is_active: !announcement.is_active
    })
    showMessage(announcement.is_active ? '公告已禁用' : '公告已启用', 'success')
    fetchAnnouncements()
  } catch (error: any) {
    showMessage(error.response?.data?.detail || '操作失败', 'error')
  }
  ...
  <div class="space-y-6">
    ...
    >
      <div
        v-if="message"
        :class="[
          'fixed top-4 right-4 z-50 px-5 py-3 rounded-xl shadow-2xl backdrop-blur-sm',
          messageType === 'success' 
