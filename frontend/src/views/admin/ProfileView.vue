      ...
      <div class="p-6">
        ...
        >
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                autocomplete="name"
                :disabled="!canEdit"
                class="w-full px-3 py-2 text-sm bg-gray-100 dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded-lg text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              >
            </div>
            <div>
              <label
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2 mb-2">
              ...
              >
                ...
                <button
                  type="button"
                  :class="['hover:text-red-400', !canEdit && 'opacity-50 cursor-not-allowed']"
                  @click="canEdit ? removeTag(index) : warnReadonly('删除网站资料标签')"
                >×</button>
              </span>
            </div>
            <div class="flex gap-2">
              <input
                ...
                autocomplete="off"
                :disabled="!canEdit"
                class="flex-1 px-3 py-2 text-sm bg-gray-100 dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded-lg text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                placeholder="输入标签"
                @keyup.enter="addTag"
              >
              <button
                type="button"
                :class="['px-3 py-2 text-white text-sm rounded-lg', canEdit ? 'bg-primary' : 'bg-primary/50 cursor-not-allowed']"
                @click="canEdit ? addTag() : warnReadonly('添加网站资料标签')"
              >
                添加
              </button>
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              :disabled="!canEdit"
              class="w-full px-3 py-2 text-sm bg-gray-100 dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded-lg text-gray-900 dark:text-white focus:border-primary focus:outline-none resize-none disabled:opacity-50 disabled:cursor-not-allowed"
            />
          </div>
        </form>
        ...
        >
          ...
          >
            <div class="flex items-center justify-between mb-2">
              <input :id="'input-tech-category-' + index"
                v-model="tech.category"
                type="text"
                :name="'tech-category-' + index"
                autocomplete="off"
                :disabled="!canEdit"
                class="px-2 py-1 text-sm bg-white dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                placeholder="分类名称"
              >
              <button
                type="button"
                :class="['text-red-400 hover:text-red-300 text-sm', !canEdit && 'opacity-50 cursor-not-allowed']"
                @click="canEdit ? removeTechCategory(index) : warnReadonly('删除网站资料技术分类')"
              >
                删除
              </button>
            </div>
            <input :id="'input-tech-items-' + index"
              v-model="tech.items"
              type="text"
              :name="'tech-items-' + index"
              autocomplete="off"
              :disabled="!canEdit"
              class="w-full px-2 py-1 text-sm bg-white dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div class="p-4 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
            <div class="grid grid-cols-2 gap-2 mb-2">
              <input id="input-newTechCategory"
                ...
                placeholder="分类名称"
              >
              <input id="input-newTechItems"
                v-model="newTechItems"
                type="text"
                name="new-tech-items"
                autocomplete="off"
                :disabled="!canEdit"
                class="px-2 py-1 text-sm bg-gray-100 dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                placeholder="技术项（逗号分隔）"
              >
            </div>
            <button
              type="button"
              :class="['w-full py-2 text-sm text-primary border border-primary/30 rounded-lg', canEdit ? 'hover:bg-primary/10' : 'opacity-50 cursor-not-allowed']"
              @click="canEdit ? addTechCategory() : warnReadonly('添加网站资料技术分类')"
            >
              + 添加技术分类
            </button>
          </div>
        ...
        >
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <button
                type="button"
                :class="['text-red-400 hover:text-red-300 text-sm', !canEdit && 'opacity-50 cursor-not-allowed']"
                @click="canEdit ? removeJourney(index) : warnReadonly('删除网站资料职业经历')"
              >
                删除
              </button>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <input :id="'input-item-period-' + index"
                v-model="item.period"
                type="text"
                :name="'journey-period-' + index"
                autocomplete="off"
                :disabled="!canEdit"
                class="px-2 py-1 text-sm bg-white dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                placeholder="时间段"
              >
              <input :id="'input-item-company-' + index"
                v-model="item.company"
                type="text"
                :name="'journey-company-' + index"
                autocomplete="off"
                :disabled="!canEdit"
                class="px-2 py-1 text-sm bg-white dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                placeholder="公司"
            ...
            <input :id="'input-item-position-' + index"
              v-model="item.position"
              type="text"
              :name="'journey-position-' + index"
              autocomplete="off"
              :disabled="!canEdit"
              class="w-full px-2 py-1 text-sm bg-white dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              placeholder="职位"
            >
            <textarea :id="'textarea-item-achievements-' + index"
              v-model="item.achievements"
              rows="2"
              :name="'journey-achievements-' + index"
              :disabled="!canEdit"
              class="w-full px-2 py-1 text-sm bg-white dark:bg-dark-100 border border-gray-200 dark:border-white/10 rounded text-gray-900 dark:text-white focus:border-primary focus:outline-none resize-none disabled:opacity-50 disabled:cursor-not-allowed"
              placeholder="主要成就"
            />
          </div>
          ...
          <button
            type="button"
            :class="['w-full py-2 text-sm text-primary border border-primary/30 rounded-lg', canEdit ? 'hover:bg-primary/10' : 'opacity-50 cursor-not-allowed']"
            @click="canEdit ? addJourney() : warnReadonly('添加网站资料职业经历')"
          >
            + 添加职业经历
          </button>
        </form>
        ...
        >
          ...
          <button
            ...
            type="button"
            :class="['w-full py-2 text-sm text-primary border border-primary/30 rounded-lg', canEdit ? 'hover:bg-primary/10' : 'opacity-50 cursor-not-allowed']"
            @click="canEdit ? initEducation() : warnReadonly('添加网站资料教育背景')"
          >
            + 添加教育背景
          </button>
        </form>
          ...
          <div class="flex flex-wrap gap-2">
            ...
            >
              ...
              <button
                type="button"
                :class="['hover:text-red-400', !canEdit && 'opacity-50 cursor-not-allowed']"
                @click="canEdit ? removeExplorationArea(index) : warnReadonly('删除网站资料探索方向')"
              >×</button>
            </span>
          </div>
          <div class="flex gap-2">
            ...
            <button
              type="button"
              :class="['px-3 py-2 text-white text-sm rounded-lg', canEdit ? 'bg-primary' : 'bg-primary/50 cursor-not-allowed']"
              @click="canEdit ? addExplorationArea() : warnReadonly('添加网站资料探索方向')"
            >
              添加
            </button>
          </div>
