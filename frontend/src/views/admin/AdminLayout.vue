...
import { useAuthStore, useSiteConfigStore, useUserProfileStore, useThemeStore } from '@/stores'
...
const menuItems = [
  ...
  { name: '公告管理', path: '/admin/announcements', icon: 'announcement' },
...
<template>
  <div class="min-h-screen bg-gray-100 dark:bg-dark-100 flex overflow-x-hidden">
    <aside
      :class="[
        'fixed lg:static inset-y-0 left-0 z-50 w-52 bg-white dark:bg-dark-100 border-r border-gray-200 dark:border-white/10 transform transition-transform duration-300 flex-shrink-0',
    ...
    <div class="flex-1 flex flex-col min-h-screen min-w-0 overflow-x-hidden">
      <header class="sticky top-0 z-40 bg-white/80 dark:bg-dark-100/80 backdrop-blur-lg border-b border-gray-200 dark:border-white/10 px-3 sm:px-4 py-3">
      ...
      <main class="flex-1 p-3 sm:p-4 overflow-x-hidden">
        <router-view />
      </main>
    </div>
