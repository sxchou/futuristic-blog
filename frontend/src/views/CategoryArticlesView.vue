...
const fetchArticles = async (page: number = 1, updateUrl: boolean = true, shouldScroll: boolean = false) => {
  ...
  let category = blogStore.getCategoryBySlug(slug)
  if (!category) {
    // 分类列表可能尚未加载完成，加载后再查找一次
    await blogStore.fetchCategories()
    category = blogStore.getCategoryBySlug(slug)
  }
    ...
    <main class="flex-1 min-w-0 lg:order-2">
      <div class="mb-8">
        ...
        <p class="text-sm text-gray-400 mt-2 flex items-center gap-2">
          <span>共 {{ blogStore.pagination.total }} 篇文章</span>
          <span
            v-if="loading"
