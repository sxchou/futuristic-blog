    ...
    <div class="flex-1 min-w-0">
      <div class="mb-8">
        <div class="flex items-center gap-2 mb-2">
          <router-link to="/categories" class="text-sm text-gray-400 hover:text-primary transition-colors">分类</router-link>
          <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
          <span class="text-sm text-gray-600 dark:text-gray-300">{{ currentCategory()?.name }}</span>
        </div>
        <div class="flex items-center gap-3">
          <div v-if="currentCategory()" class="w-10 h-10 rounded-xl flex items-center justify-center" :style="{ backgroundColor: currentCategory()!.color + '15' }">
            <svg class="w-5 h-5" :style="{ color: currentCategory()!.color }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
            </svg>
          </div>
          <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">{{ currentCategory()?.name || '分类文章' }}</h1>
            <p v-if="currentCategory()?.description" class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ currentCategory()?.description }}</p>
          </div>
        </div>
        <p class="text-sm text-gray-400 mt-2">共 {{ blogStore.pagination.total }} 篇文章</p>
      </div>
