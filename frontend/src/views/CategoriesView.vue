...
<template>
  <div class="flex flex-col lg:flex-row gap-8">
    <div class="flex-1 min-w-0">
      <div class="mb-8">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2">分类目录</h1>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p v-if="category.description" class="text-gray-500 dark:text-gray-400 text-sm mb-2 line-clamp-2">
              {{ category.description }}
            </p>
            <span class="text-xs font-medium" :style="{ color: category.color }">
              {{ category.article_count }} 篇文章
            </span>
          </div>
          <svg class="w-5 h-5 text-gray-300 dark:text-gray-600 group-hover:text-primary group-hover:translate-x-0.5 transition-all flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
          </svg>
        </router-link>
      </div>

      <div v-if="blogStore.categories.length === 0" class="text-center py-16">
