...
<template>
  <div class="flex flex-col lg:flex-row gap-8">
    <div class="flex-1 min-w-0">
      <div class="mb-8">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2">关于我</h1>
      ...
      <template v-else-if="profile">
        <div class="glass-card p-6 md:p-8 mb-6">
          <div class="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div class="flex-1 text-center sm:text-left">
              <h2 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1">{{ profile.name }}</h2>
              <p v-if="profile.alias" class="text-base text-gray-400 mb-2">{{ profile.alias }}</p>
              <p v-if="profile.slogan" class="text-primary font-medium text-sm mb-3">{{ profile.slogan }}</p>
              <div v-if="profile.tags?.length" class="flex flex-wrap justify-center sm:justify-start gap-1.5 mb-3">
                <span
              ...
              <p v-if="profile.bio" class="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">{{ profile.bio }}</p>
            </div>
          </div>
        </div>

        <div v-if="profile.tech_stack?.length" class="glass-card p-6 mb-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-2.5">{{ stack.category }}</h3>
              <div class="flex flex-wrap gap-1.5">
                <span v-for="item in stack.items" :key="item" class="tag-badge bg-gray-100 dark:bg-dark-200 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-white/10">
                  {{ item }}
                </span>
              </div>
            </div>
        ...
        <div v-if="profile.journey?.length" class="glass-card p-6 mb-6">
          <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
          ...
          <div class="relative">
            ...
            >
              ...
              <div class="glass-card-hover p-4">
                <div class="text-primary font-medium text-sm mb-0.5">{{ item.period }}</div>
                <h3 class="text-base font-bold text-gray-900 dark:text-white mb-0.5">{{ item.position }}</h3>
                <div class="text-accent text-sm mb-1.5">{{ item.company }}</div>
                <p class="text-gray-500 dark:text-gray-400 text-sm">{{ item.achievements }}</p>
              </div>
            </div>
          </div>
        </div>

        <div v-if="profile.education" class="glass-card p-6 mb-6">
          <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
          ...
          <div class="glass-card-hover p-4 inline-block">
            <div class="text-primary font-medium text-sm mb-0.5">{{ profile.education.period }}</div>
            <h3 class="text-base font-bold text-gray-900 dark:text-white mb-0.5">{{ profile.education.school }}</h3>
            <div class="text-gray-600 dark:text-gray-300 text-sm">{{ profile.education.major }} · {{ profile.education.degree }}</div>
          </div>
        </div>

        <div v-if="profile.exploration_areas?.length" class="glass-card p-6 mb-6">
          <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
        ...
        <div v-if="socialLinksStore.hasSocialLinks" class="glass-card p-6">
          <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <svg class="w-5 h-5 text-cyber-pink" fill="none" stroke="currentColor" viewBox="0 0 24 24">
