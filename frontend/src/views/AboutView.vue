...
const fetchProfile = async (force = false) => {
  ...
  if (!force && profileCache && now - profileCache.timestamp < CACHE_TTL) {
    profile.value = profileCache.data
  ...
  try {
    ...
    if (profile.value) {
      profileCache = { data: profile.value, timestamp: now }
      hasError.value = false
    }
  } catch (error: unknown) {
    if (isCancelError(error)) {
      if (!profile.value) {
        isLoading.value = true
        setTimeout(() => fetchProfile(true), 300)
      }
      return
    }
  ...
  <div class="flex flex-col lg:flex-row gap-6">
    ...
    <div class="flex-1 min-w-0 lg:order-2">
      ...
      >
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-4">加载失败，请稍后重试</p>
        <button
