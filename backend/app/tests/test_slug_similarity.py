...
class TestGenerateSlugWithFallback:
    def test_exact_match(self):
        ...
        result = asyncio.run(generate_slug_with_fallback("Hello World", existing))
    ...
    def test_similar_match(self):
        existing = ["hello-world-test", "another-slug"]
        result = asyncio.run(generate_slug_with_fallback("Hello World", existing, similarity_threshold=0.5))
        assert result['source'] == "similar_match"
        assert result['similarity_score'] >= 0.5
    
    def test_generate_new_slug(self):
        existing = ["completely-different", "another-slug"]
        result = asyncio.run(generate_slug_with_fallback("Hello World", existing, similarity_threshold=0.9))
        assert result['source'] == "generated"
        assert result['slug'] == "hello-world"
    
    def test_empty_input(self):
        existing = ["hello-world"]
        result = asyncio.run(generate_slug_with_fallback("", existing))
        assert result['slug'] == "untitled"
        assert result['source'] == "default"
    
    def test_empty_existing_list(self):
        result = asyncio.run(generate_slug_with_fallback("Hello World", []))
        assert result['slug'] == "hello-world"
        assert result['source'] == "generated"
...
class TestGenerateSlug:
    def test_english_text(self):
        slug = asyncio.run(generate_slug("Hello World"))
        assert slug == "hello-world"
    
    def test_special_characters(self):
        slug = asyncio.run(generate_slug("Hello! World? #Test"))
    ...
    def test_chinese_text(self):
        slug = asyncio.run(generate_slug("你好世界"))
        assert bool(slug)
        assert not any('\u4e00' <= c <= '\u9fff' for c in slug)
    
    def test_mixed_text(self):
        slug = asyncio.run(generate_slug("Hello 世界"))
        assert "hello" in slug
    
    def test_empty_string(self):
        slug = asyncio.run(generate_slug(""))
        assert slug == ""
    
    def test_max_length(self):
        long_text = "a" * 200
        slug = asyncio.run(generate_slug(long_text, max_length=50))
        assert len(slug) <= 50
...
class TestIncompleteTitleScenarios:
    def test_partial_title_prefix_match(self):
        ...
        result = asyncio.run(generate_slug_with_fallback("regular", existing, similarity_threshold=0.5))
    ...
    def test_partial_title_word_overlap(self):
        ...
        result = asyncio.run(generate_slug_with_fallback("javascript", existing, similarity_threshold=0.5))
        assert result['matched_slug'] == "javascript-tutorial"
    
    def test_typo_in_title(self):
        existing = ["machine-learning", "deep-learning"]
        result = asyncio.run(generate_slug_with_fallback("machne-learning", existing, similarity_threshold=0.7))
    ...
    def test_abbreviated_title(self):
        ...
        result = asyncio.run(generate_slug_with_fallback("frontend", existing, similarity_threshold=0.5))
...
class TestEdgeCases:
    def test_very_long_input(self):
        long_input = "a" * 1000
        existing = ["a" * 100]
        result = asyncio.run(generate_slug_with_fallback(long_input, existing))
        assert result['slug'] is not None
    
    def test_unicode_characters(self):
        existing = ["test-slug"]
        result = asyncio.run(generate_slug_with_fallback("你好🌟世界", existing))
        assert result['slug'] is not None
    
    def test_numbers_in_title(self):
        existing = ["python-3-tutorial"]
        result = asyncio.run(generate_slug_with_fallback("python 3", existing, similarity_threshold=0.5))
    ...
    def test_threshold_boundary(self):
        ...
        result_low = asyncio.run(generate_slug_with_fallback("hello", existing, similarity_threshold=0.3))
        result_high = asyncio.run(generate_slug_with_fallback("hello", existing, similarity_threshold=0.99))
