...
def send_email_with_config(
    ...
    try:
        result = _send_smtp_email(config, to_email, subject, html_content)
        log.status = 'sent'
...
async def update_email_config(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission("email.edit"))
):
...
async def activate_email_config(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission("email.activate"))
):
    config = db.query(EmailConfig).filter(EmailConfig.id == config_id).first()
    if not config:
        raise HTTPException(status_code=404, detail="配置不存在")
...
async def delete_email_config(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission("email.delete"))
):
    config = db.query(EmailConfig).filter(EmailConfig.id == config_id).first()
    if not config:
        raise HTTPException(status_code=404, detail="配置不存在")
...
async def test_email_config(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission("email.test"))
):
    config = get_active_email_config(db)
    if not config:
        raise HTTPException(
    ...
    if result['success']:
        ...
        if 'Network is unreachable' in error_msg or 'Connection refused' in error_msg or '连接错误' in error_msg:
            friendly_error = "网络连接失败，请检查：1. 服务器网络是否正常 2. SMTP服务器地址是否正确 3. 防火墙是否允许访问"
        elif 'SMTP认证失败' in error_msg or 'Authentication' in error_msg:
            friendly_error = "SMTP认证失败，请检查：1. 邮箱地址是否正确 2. 授权码/密码是否正确 3. 是否开启了SMTP服务"
        elif 'SSL' in error_msg or 'TLS' in error_msg:
            friendly_error = "SSL/TLS连接失败，请检查：1. 端口号是否正确（SSL通常使用465，TLS通常使用587）2. 服务器是否支持加密连接"
        elif 'timeout' in error_msg.lower() or '超时' in error_msg:
            friendly_error = "连接超时，请检查：1. 网络连接是否稳定 2. SMTP服务器地址是否正确 3. 端口是否被阻止"
        else:
            friendly_error = f"邮件发送失败: {error_msg}"
...
async def switch_email_provider(
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission("email.switch_provider"))
):
    import json
...
async def test_resend_email(
    ...
    try:
        ...
        logger.info(f"Resend email sent successfully to {test_data.recipient_email}, Message ID: {result.get('message_id')}")
