...
async def upload_file(
    ...
    required_permission = "article.upload_image" if is_image else "article.upload_file"
    if not PermissionService.has_permission(db, current_user.id, required_permission):
        raise HTTPException(
            status_code=403, 
            detail=f"权限不足：需要「{'上传图片' if is_image else '上传附件'}」权限"
        )
    ...
    LogService.log_operation(
        ...
        action="upload",
        module="file",
        description=f"上传文件: {file.filename} ({format_file_size(file_size)})",
        target_type="file",
...
async def upload_image(
    ...
    article_id: Optional[int] = Form(None),
    db: Session = Depends(get_db),
    current_user = Depends(require_permission("article.upload_image"))
):
    ensure_upload_dir()
    ...
    LogService.log_operation(
        ...
        action="upload_image",
        module="file",
        description=f"上传图片: {file.filename} ({format_file_size(file_size)})",
        target_type="file",
        target_id=db_file.id,
        request=request
    )
...
async def delete_file(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user = Depends(require_permission("storage.delete"))
):
    db_file = db.query(ArticleFile).filter(ArticleFile.id == file_id).first()
    if not db_file:
        raise HTTPException(status_code=404, detail="文件不存在")
...
async def update_file_order(
    ...
    LogService.log_operation(
        ...
        action="update_order",
        module="file",
        description=f"更新文件排序 ({len(data.orders)}个文件)",
        request=request
    )
...
async def get_storage_info(
    db: Session = Depends(get_db),
    current_user = Depends(require_permission("storage.view"))
):
    result = {
...
async def delete_orphan_files(
    ...
    EXCLUDED_DIRS = {'avatars', 'logos', 'images'}
    ...
    LogService.log_operation(
        ...
        module="file",
        description=f"清理孤立文件: 删除{len(deleted_files)}个 ({format_file_size(deleted_size)})",
        request=request
    )
...
async def update_cache_control(
    ...
    LogService.log_operation(
        ...
        user_id=current_user.id,
        username=current_user.username,
        action="update_cache_control",
        module="file",
        description=f"更新缓存策略: {result['updated']}个文件成功, {result['failed']}个失败",
        request=request
    )
...
async def update_single_cache_control(
    ...
    LogService.log_operation(
        ...
        user_id=current_user.id,
        username=current_user.username,
        action="update_single_cache_control",
        module="file",
        description=f"更新单个文件缓存策略: {db_file.original_filename}",
        target_type="file",
        target_id=file_id,
        request=request
    )
