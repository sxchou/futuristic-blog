...
from app.schemas import (
    ...
    SiteConfigResponse, AnnouncementResponse, UserProfileResponse, UserResponse
)
...
def _get_categories_cached() -> List[CategoryResponse]:
    ...
    try:
        ...
        if cached:
            result = []
            for c in cached:
                if isinstance(c, CategoryResponse):
                    result.append(c)
                elif isinstance(c, dict):
                    result.append(CategoryResponse(**c))
                else:
                    result.append(CategoryResponse.model_validate(c))
            return result
...
def _get_tags_cached() -> List[TagResponse]:
    ...
    try:
        ...
        if cached:
            result = []
            for t in cached:
                if isinstance(t, TagResponse):
                    result.append(t)
                elif isinstance(t, dict):
                    result.append(TagResponse(**t))
                else:
                    result.append(TagResponse.model_validate(t))
            return result
...
def _get_announcements_cached(active_only: bool = True) -> List[AnnouncementResponse]:
    ...
    try:
        ...
        if cached:
            result = []
            for a in cached:
                if isinstance(a, AnnouncementResponse):
                    result.append(a)
                elif isinstance(a, dict):
                    result.append(AnnouncementResponse(**a))
                else:
                    result.append(AnnouncementResponse.model_validate(a))
            return result
...
def _get_github_stats_cached() -> Dict[str, Any]:
    ...
    try:
        ...
        show_github_config = db.query(SiteConfig).filter(SiteConfig.key == "show_github_stats").first()
        show_github = show_github_config.value == "true" if show_github_config else False
    ...
    try:
        ...
        with httpx.Client(timeout=10) as client:
            ...
            if response.status_code == 200:
                ...
                cache_manager.set("site_config", "github_stats", {"data": result, "timestamp": time.time()})
    except Exception as e:
        logger.warning(f"Failed to fetch GitHub stats async: {e}")
...
async def get_init_data(
    ...
    try:
        ...
        if isinstance(github_stats, dict) and github_stats.get("pending"):
            ...
            if repo_url:
                loop.run_in_executor(executor, _fetch_github_stats_async, repo_url)
