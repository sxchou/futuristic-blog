...
def get_site_configs_cached(db: Session) -> List[SiteConfigResponse]:
...
def get_categories_cached(db: Session) -> List[CategoryResponse]:
...
def get_tags_cached(db: Session) -> List[TagResponse]:
    cache_key = "all_tags"
...
def get_announcements_cached(db: Session, active_only: bool = True) -> List[AnnouncementResponse]:
...
def get_articles_list(db: Session, page: int, page_size: int, is_featured: Optional[bool] = None) -> PaginatedResponse:
...
def get_github_stats_cached(db: Session) -> Dict[str, Any]:
    import os
    import httpx
...
async def get_init_data(
    ...
    site_config = get_site_configs_cached(db)
    categories = get_categories_cached(db)
    tags = get_tags_cached(db)
    announcements = get_announcements_cached(db, active_only=True)
    articles = get_articles_list(db, page, page_size)
    featured_articles = get_articles_list(db, 1, featured_page_size, is_featured=True)
    github_stats = get_github_stats_cached(db)
    ...
    if current_user:
        user_profile = get_user_profile_data(db, current_user)
