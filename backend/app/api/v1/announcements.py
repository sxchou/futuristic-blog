...
def create_announcement(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user = Depends(require_permission("announcement.create"))
):
...
def update_announcement(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user = Depends(require_permission("announcement.edit"))
):
    announcement = db.query(Announcement).filter(Announcement.id == announcement_id).first()
    if not announcement:
        raise HTTPException(status_code=404, detail="公告不存在")
    
    old_is_active = announcement.is_active
    ...
    if 'is_active' in update_data and len(update_data) == 1:
        if old_is_active != announcement.is_active:
            if announcement.is_active:
                description = f"启用公告: {announcement.title}"
                action = "启用"
            else:
                description = f"禁用公告: {announcement.title}"
                action = "禁用"
...
def delete_announcement(
    ...
    request: Request,
    db: Session = Depends(get_db),
    current_user = Depends(require_permission("announcement.delete"))
):
    announcement = db.query(Announcement).filter(Announcement.id == announcement_id).first()
    if not announcement:
        raise HTTPException(status_code=404, detail="公告不存在")
