...
from app.models import User, EmailLog, NotificationSettings, PasswordReset, RefreshToken, Role, user_roles
...
async def login(
    ...
    if not user.is_verified:
        ...
        is_admin = user.id == 1 or any(r.code in ('super_admin', 'admin') for r in roles)
        if not is_admin:
            background_tasks.add_task(
                log_login_sync,
                user.id,
    ...
    if not user:
        ...
        if email_log:
            ...
            if verified_user and verified_user.is_verified:
                ...
                roles = PermissionService.get_user_roles(db, verified_user.id)
                roles_data = [{"id": r.id, "name": r.name, "code": r.code} for r in roles] if roles else []
                ...
                return {
                    ...
                    "user": {
                        ...
                        "is_admin": verified_user.id == 1 or any(r.code in ('super_admin', 'admin') for r in roles),
                        "is_verified": verified_user.is_verified,
                        "has_password": bool(verified_user.hashed_password),
...
async def get_me(
    ...
    is_admin = current_user.id == 1 or any(r.code in ('super_admin', 'admin') for r in roles)
    ...
    user_dict = {
        ...
        "verification_token_expires": current_user.verification_token_expires,
        "roles": roles_data
    }
...
async def refresh_token(
    ...
    if not user.is_verified:
        _roles = PermissionService.get_user_roles(db, user.id)
        _is_admin = user.id == 1 or any(r.code in ('super_admin', 'admin') for r in _roles)
        if not _is_admin:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="账户未验证"
