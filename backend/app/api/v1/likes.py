...
async def get_user_liked_articles(
    ...
    for like in likes:
        ...
        ).filter(Article.id == like.article_id).first()
        if article:
...
async def toggle_like(
    ...
    if existing_like:
        ...
        background_tasks.add_task(send_like_notification_bg, article_title, article_slug, liker_name, author_email, author_name)
