...
def get_github_cache():
    return cache_manager.get(CACHE_NAME, "github_stats") or {
        "data": None,
        "timestamp": 0,
        "repo_url": ""
    }
...
def set_github_cache(data: dict):
    cache_manager.set(CACHE_NAME, "github_stats", data, ttl=GITHUB_CACHE_TTL)


def get_rate_limit_cache():
    return cache_manager.get(CACHE_NAME, "github_rate_limit") or {
        "reset_time": 0
    }


def set_rate_limit_cache(reset_time: float):
    cache_manager.set(CACHE_NAME, "github_rate_limit", {"reset_time": reset_time}, ttl=3600)
...
async def get_github_stats(db: Session = Depends(get_db)):
    ...
    if current_time < rate_limit_cache["reset_time"]:
        logger.info(f"GitHub API rate limited, using cached data. Reset in {int(rate_limit_cache['reset_time'] - current_time)}s")
    ...
    if (github_cache.get("data") and 
        github_cache.get("repo_url") == repo_url and 
        current_time - github_cache.get("timestamp", 0) < GITHUB_CACHE_TTL):
        return {"enabled": True, **github_cache["data"]}
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 200:
                ...
                if rate_limit_reset:
                    set_rate_limit_cache(int(rate_limit_reset))
...
def update_site_config(
    ...
    if key == "github_repo_url":
        set_github_cache({"data": None, "timestamp": 0, "repo_url": ""})
